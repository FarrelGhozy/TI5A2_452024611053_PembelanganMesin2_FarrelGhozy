...
Seluruh eksperimen menggunakan konfigurasi yang seragam: optimizer Adam, \emph{cross-entropy loss} (\texttt{torch.nn.CrossEntropyLoss}), batch size 32, early stopping dengan patience 4 (monitor validation loss dan memulihkan bobot terbaik), serta ReduceLROnPlateau (faktor 0.5, patience 2, minimum $10^{-6}$). Training menggunakan mixed precision (FP16) pada GPU NVIDIA RTX 4060. Detail learning rate dan skenario eksperimen dijelaskan pada Bagian IV.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{../v2/results/e2/top_confusions.png}
\caption{Enam pasangan kesalahan klasifikasi terbanyak MobileNetV3-Small (E2) pada set uji; panah menunjukkan kelas sebenarnya $\rightarrow$ prediksi.}
\label{fig:cm}
\end{figure}
